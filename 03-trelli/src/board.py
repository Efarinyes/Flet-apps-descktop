...
class Board(ft.Container):
    ...
    def __init__(self, app, store: DataStore, name: str, page: ft.Page):
        ...
        self.add_list_button = ft.FloatingActionButton(
            icon=ft.Icons.ADD, text="afegir una llista", height=30, on_click=self.create_list
        )
    ...
    def create_list(self, e):
        ...
        def close_dlg(e):
            if (hasattr(e.control, "text") and not e.control.text == "Cancel·lar") or (
        ...
        dialog_text = ft.TextField(
            label="Nom de la nova llista", on_submit=close_dlg, on_change=textfield_change
        )
        create_button = ft.ElevatedButton(
            text="Crear", bgcolor=ft.Colors.BLUE_200, on_click=close_dlg, disabled=True
        )
        dialog = ft.AlertDialog(
            title=ft.Text("Nom de la nova llista"),
            content=ft.Column(
                [
                    ft.Container(
                    ...
                    ft.Row(
                        [
                            ft.ElevatedButton(text="Cancel·la", on_click=close_dlg),
                            create_button,
                        ],
