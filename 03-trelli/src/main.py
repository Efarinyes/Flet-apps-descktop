...
class TrelloApp(AppLayout):
    ...
    def initialize(self):
        ...
        if len(self.boards) == 0:
            self.create_new_board("Primer tauler")
        self.page.go("/")

    def login(self, e):
        def close_dlg(e):
            if user_name.value == "" or password.value == "":
                user_name.error_text = "Si us plau, proporcioneu el nom d'usuari"
                password.error_text = "Si us plau, proporcioneu una contrassenya"
                self.page.update()
                return
            else:
                ...
                self.page.client_storage.set("usuari actual", user_name.value)

            self.page.close(dialog)
            self.appbar_items[0] = ft.PopupMenuItem(
                text=f"{self.page.client_storage.get('usuari actual')}'s Profile"
            )
            self.page.update()

        user_name = ft.TextField(label="Nom d'usuari")
        password = ft.TextField(label="Contrasenya", password=True)
        dialog = ft.AlertDialog(
            title=ft.Text("Si us plau, introduïu les vostres credencials d'inici de sessió."),
            content=ft.Column(
                [
                    user_name,
                    password,
